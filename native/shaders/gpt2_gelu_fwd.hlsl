...
void CSMain(uint3 tid : SV_DispatchThreadID) {
    ...
    const float inner = x + COEFF * x * x * x;
    // Clamp tanh argument: Intel iGPU tanh(k) overflows to NaN for |k|>~88
    // (driver uses (exp(2k)-1)/(exp(2k)+1), Inf/Inf = NaN).
    // tanh saturates to sign(k) for |k|>5, so clamping at ±15 is numerically exact.
    const float k = clamp(SQRT_2_OVER_PI * inner, -15.0f, 15.0f);
